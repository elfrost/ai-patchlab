...
- `docs/` — GitHub Pages site (Jekyll, theme `cayman`): `_config.yml`, `index.md` (landing: intro, six featured scans, compact 83-row table), `scan-log.md` (full prose archive of every scan entry), `fixed.md` + `clean.md` (curated collections generated from the index table — never from keyword inference), `scans/` (per-scan write-ups), top-level pages (`work-with-me.md` conversion funnel, `daybreak-and-local-first.md` positioning essay vs OpenAI Daybreak), `templates/scan-post.md` (template, excluded from publish)
