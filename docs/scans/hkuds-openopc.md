...
- **Semgrep coverage was partial and, as usual, the truth was in the `errors` array, not `paths.skipped` (which was 0).** 22 rules timed out and three Python files failed to parse entirely. The largest module in the codebase — a ~440 KB, ~9,000-line file — had several rules time out on it, and it is exactly the kind of file where a hand read pays for itself. Every real item in this series' history has come from reading, not from a rule; this scan is another vote for the standing backlog item that a timed-out rule on a large, central file must be surfaced as a coverage gap, not silence.
