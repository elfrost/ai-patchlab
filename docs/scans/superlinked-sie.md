...
- 2026-09-18 — **hardened again, by the maintainer**, in [#305](https://github.com/superlinked/sie/pull/305) (`27ae3a18`). #292's new trust test split and prefix-matched the `Host` string instead of parsing it, so a `Host` carrying userinfo passed as trusted: `localhost:8088@evil.example` read as loopback, and the metadata built from it resolves to `evil.example`. #305 parses `Host` as `uri-host [ ":" port ]` (RFC 9110 §7.2) before trusting anything — userinfo, paths and out-of-range ports are refused, bracketed IPv6 is validated — and its description documents a sibling sweep of every allowlist in the repository, including a note that the MCP SDK's transport-security middleware matches wildcard ports the same way. My own re-test of #292 had turned up the same gap on 09-17; the write-up was still an unsent draft when #305 landed, so the credit is theirs.
- 2026-09-18 — re-verified with one differential run against both trees, on h11 and on the `uvicorn[standard]` httptools stack: before #305, all five userinfo `Host` values advertised the attacker's host on both metadata routes; after it, none do, the other malformed values the old test admitted (paths, a backslash, a `#`, a bracket suffix, ports `0`, `65536`, `99999`, `abc`) get the 503, and every legitimate host — loopback, `[::1]` with a port, uppercase, a wildcard entry with a port — still resolves
